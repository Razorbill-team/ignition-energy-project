select * from (select * from (SELECT
  ...
  f.farm_name ,
  f.farmid Info,
TRIM(st.status_name) AS status_text,
  r."ActivePower" active_power,
  -- r.rated_power rated_power ,
  r."ReactivePower" reactive_power,
  r."PowerFactor", 
  case when hist.setpointid=1 then setpoint_value else 0 end as Economical_Set_Point ,
  case when hist.setpointid=2 then setpoint_value else 0 end as Technical_Set_Point,
  '' Power_Factor_Set_Point,
 -- r."Voltage_U12",
 -- r."Current_I1",  
...
JOIN status_summary st	
    ON st.farmid=f.farmid
...
left join integration_schema.SETPOINTS_HISTORICAL_TABLE hist
on f.farmid = hist.farmid 
WHERE
